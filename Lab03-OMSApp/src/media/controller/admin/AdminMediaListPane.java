...
public class AdminMediaListPane extends ADataListPane<Media>{
	...
	public void decorateSinglePane(ADataSinglePane<Media> singlePane) {
		...
		JButton button = new JButton("Edit product");
		...
		button.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (controller instanceof AdminMediaPageController) {
					((AdminMediaPageController)controller).editProductInformation(singlePane.getData());
					
					//((AdminMediaPageController) controller).addToCart(singlePane.getData().getId(), singlePane.getData().getTitle(), singlePane.getData().getCost(), (int)spin.getValue());
				}
			}
		});
